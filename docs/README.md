# Content Sections Docs

Content Sections provides reusable content sections for Capell admin and frontend surfaces.

Start at the [package README](../README.md) when deciding whether to install this package. Use the docs below for setup, extension, debugging, and verification details.

## Guides

| Doc                       | Use it for                                                                         |
| ------------------------- | ---------------------------------------------------------------------------------- |
| [Overview](overview.md) | Package workflow, runtime surfaces, install notes, verification, and public-rendering boundaries. |

## Read Next

| Related doc                                             | Why                                                   |
| ------------------------------------------------------- | ----------------------------------------------------- |
| [Repository package docs](../../../docs/README.md)      | Cross-package workflow index and install-order notes. |
| [Layout Builder](../../layout-builder/docs/overview.md) | Neighboring package in the same Capell workflow.      |
| [Widget Library](../../widget-library/docs/overview.md) | Neighboring package in the same Capell workflow.      |
